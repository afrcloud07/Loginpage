...
function drawAnalogClock(canvas) {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    ctx.fillText(String(i), numX, numY);
  }
  ...
  drawHand(ctx, radius, (hour + minute / 60) * (Math.PI / 6), radius * 0.4, 4 * scale, "#FFFFFF");
  drawHand(ctx, radius, (minute + second / 60) * (Math.PI / 30), radius * 0.6, 3 * scale, "#FFFFFF");
  drawHand(ctx, radius, (second * Math.PI) / 30, radius * 0.65, 1.5 * scale, "#FF4444");
...
export default function AnalogClockCanvas({ width = 500, height = 500 }) {
  ...
  useEffect(() => {
    ...
    const tick = () => drawAnalogClock(canvas);
    tick();
    const t = setInterval(tick, 1000);
    return () => clearInterval(t);
  }, []);

  return <canvas id="analogClock" ref={ref} width={width} height={height} />;
}
